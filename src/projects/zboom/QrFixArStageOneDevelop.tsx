...
const ARIntegrated: React.FC = () => {
    ...
    const [markers, setMarkers] = useState<{ id: string; content: Content }[]>([]);
    ...
    const startScanning = useCallback(() => {
        const scanInterval = setInterval(() => {
            ...
            scanQRCode();
        }, 200); // افزایش interval به 200ms برای عملکرد بهتر
    ...
    const scanQRCode = useCallback(() => {
        ...
        if (code) {
            setIsScanning(false);
            fetchAndRenderContent(code.data, `qr-${Date.now()}`);
        }
    }, []);

    const fetchAndRenderContent = useCallback(async (qrData: string, markerId: string) => {
        setLoading(true);
        try {
            const newContent = await ContentManager.fetchContent(qrData);
            setMarkers((prev) => [...prev, { id: markerId, content: newContent }]);
            setContent(newContent); // برای سازگاری با کد قبلی
        } catch (err) {
    ...
    const handleManualSubmit = useCallback(
        (e: React.MouseEvent<HTMLButtonElement>) => {
            ...
            if (manualInput) {
                setIsScanning(false);
                fetchAndRenderContent(manualInput, `manual-${Date.now()}`);
            }
        },
    ...
    return (
        ...
        >
            ...
            <div className="relative rounded-lg overflow-hidden shadow-lg">
                ...
                >
                    {markers.map((marker) => (
                        <a-marker key={marker.id} type="pattern" url={`/qr-${marker.id}.patt`}>
                            {marker.content.type === 'aframe' && (
                                <a-entity dangerouslySetInnerHTML={{ __html: marker.content.content }} />
                            )}
                        </a-marker>
                    ))}
